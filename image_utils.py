...
import io
from box_utils import decode, compute_nms, compute_iou
...
def generate_detection_img(img, conf, loc, default_boxes, num_classes, idx_to_name):
    # conf = tf.squeeze(conf, 0)
    # loc = tf.squeeze(loc, 0)

    conf = tf.math.softmax(conf, axis=-1)
    classes = tf.math.argmax(conf, axis=-1)
    scores = tf.math.reduce_max(conf, axis=-1)

    # print(scores)

    boxes = decode(default_boxes, loc)
    ...
    # plt.figure()
    fig, ax = plt.subplots(1)
    img = Image.fromarray(np.uint8((img+1)*127))
    ax.imshow(img)
