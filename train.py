...
from network import create_ssdlite
from losses import create_losses
from dataset import create_batch_generator
from image_utils import generate_detection_img, plot_to_image

parser = argparse.ArgumentParser()
parser.add_argument('--data-dir', default='../dataset')
parser.add_argument('--data-type', default='VOC2007')
...
if __name__ == '__main__':
    ...
    batch_generator, val_generator, info = create_batch_generator(
        args.data_dir, args.data_type, default_boxes,
        config['image_size'],
        args.batch_size, args.num_batches,
        mode='train', augmentation=['flip']) 

    try:
        ssdlite = create_ssdlite(NUM_CLASSES, args.arch,
    ...
    for epoch in range(args.initial_epoch, args.num_epochs):
        ...
        for i, (_, imgs, gt_confs, gt_locs) in enumerate(batch_generator):
            loss, conf_loss, loc_loss, l2_loss = train_step(
                imgs, gt_confs, gt_locs, ssdlite, criterion, optimizer)
        ...
        for i, (_, imgs, gt_confs, gt_locs) in enumerate(val_generator):
            val_confs, val_locs = ssdlite(imgs)
            ...
            if i == 0:
                fig = generate_detection_img(imgs.numpy()[epoch % args.batch_size],
                                             val_confs.numpy()[epoch % args.batch_size], 
                                             val_locs.numpy()[epoch % args.batch_size], default_boxes, NUM_CLASSES, info['idx_to_name'])
        ...
        with val_summary_writer.as_default():
            tf.summary.image("detection", plot_to_image(fig), step=epoch)

        if (epoch + 1) % 10 == 0:
            ssdlite.save_weights(
                os.path.join(args.checkpoint_dir, 'ssdlite_epoch_{}.h5'.format(epoch + 1)))
